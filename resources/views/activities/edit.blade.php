    ...
    <div class="card-body">
        <form action="{{ route('activities.update', $activity) }}" method="POST">
                ...
                <div class="col-md-9">
                    <select class="form-select @error('activity_type') is-invalid @enderror" id="activity_type" name="activity_type" required>
                        ...
                        @foreach($activityTypes as $key => $value)
                            <option value="{{ $key }}" {{ old('activity_type', $activity->activity_type) == $key ? 'selected' : '' }}>{{ $value }}</option>
                        @endforeach
                    </select>
                    @error('activity_type')
            ...
            <div class="mb-3 row">
                ...
                <div class="col-md-9">
                    <input type="text" class="form-control @error('activity_name') is-invalid @enderror" id="activity_name" name="activity_name" value="{{ old('activity_name', $activity->activity_name) }}" required>
                    @error('activity_name')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="mb-3 row">
                ...
                <div class="col-md-9">
                    <input type="datetime-local" class="form-control @error('start_time') is-invalid @enderror" id="start_time" name="start_time" value="{{ old('start_time', $activity->start_time->format('Y-m-d\TH:i')) }}" required>
                    @error('start_time')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="mb-3 row">
                ...
                <div class="col-md-9">
                    <input type="datetime-local" class="form-control @error('end_time') is-invalid @enderror" id="end_time" name="end_time" value="{{ old('end_time', $activity->end_time ? $activity->end_time->format('Y-m-d\TH:i') : '') }}">
                    @error('end_time')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
